...
ref_substrains = {k: v for k, v in ref_strain.items() if k.startswith(speci)}
...
print("Precision\t" + str(len(set(sample_combined) & set(ref_specie[speci]))/len(set(sample_combined) & shared_genes)))
print("Recall\t\t" + str(len(set(sample_combined) & set(ref_specie[speci]))/len(set(ref_specie[speci]) & shared_genes)))
...
plt.colorbar()
plt.savefig(speci + '.png', dpi=1200)